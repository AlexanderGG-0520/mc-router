...
- Login, backend failure, and maintenance fallback responses.
...
- Status fallback config defaults, denied status response JSON, status ping/pong echo, disabled fallback close behavior, login route denied close behavior, default route precedence, malformed status request close behavior, and route denied metric preservation.
...
2. Add backend dial failure and timeout fallback.
