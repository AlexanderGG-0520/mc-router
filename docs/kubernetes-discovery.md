...
Invalid Service annotations are skipped and reported in the discovery `Result`. Duplicate discovered hosts are also skipped. Neither condition fails startup as long as the Kubernetes API list itself succeeded. Skipped Services, duplicate host metadata, and skipped reason counts stay on the `Result` for startup logging and metrics; they are not exposed through `RouteProvider`.

Startup skipped Service metrics are recorded from the complete startup `Result` through the startup discovery report after the startup route snapshot rebuild succeeds and the server metrics recorder exists. That `Result` metadata flows to the metrics recorder through the explicit startup discovery report boundary. It is not passed through `RouteProvider`, `SnapshotProvider`, merge data, or `RouteSnapshot`.
