...
Static routes take precedence over discovered routes, including every explicit Java `aliases` identity on a static route. `defaultRoute` remains outside the explicit route list and is evaluated after static and discovered routes. See [Kubernetes Discovery](kubernetes-discovery.md).
...
Route fields such as `aliases`, `statusBackend`, and `statusOverride` are part of the same immutable snapshot. A successful reload changes their behavior only for new connections; existing proxied connections keep their original backend.
