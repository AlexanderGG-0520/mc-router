...
Kubernetes Service annotation discovery groundwork exists for config validation, annotation parsing, and in-memory discovered route snapshot building, but it does not watch Kubernetes or feed runtime router snapshots yet. Future integration should merge route sources with static routes first, valid discovered routes next, and `defaultRoute` last. See [Kubernetes Discovery](kubernetes-discovery.md).
